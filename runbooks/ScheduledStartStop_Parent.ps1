...
function ScheduleSnoozeAction ($VMObject,[string]$Action)
{
   	...
   	if ($VMObject.Type -eq "Classic")
	{
        ...
        $currentVM = Get-AzResource -ResourceType Microsoft.ClassicCompute/virtualMachines | where Name -Like $VMObject.Name  -ErrorAction SilentlyContinue
    ...
    {  
        ...
        {   
            ...
            if(($AzureVMListClassic -ne $null) -and ($enableClassicVMs))
            {
                ...
                {
                    $currentVM = $AllClassicVMs | where Name -EQ $vmObj.Name  -ErrorAction SilentlyContinue
