...
function PerformActionOnSequencedTaggedVMAll($Sequences, [string]$Action, $TagName, $ExcludeList)
{
    ...
    foreach($seq in $Sequences)
    {
        ...
        if($WhatIf -eq $false)
        {
			...
			{
                $AzureVMList += $AllVMResources | where ResourceGroupName -eq $rg.ResourceGroupName | Select Name, ResourceGroupName
            }
            
            foreach($VM in $AzureVMList)
            {
                $FilterTagVMs = $AllVMs | where ResourceGroupName -EQ $VM.ResourceGroupName | where Name -EQ $VM.Name  
            ...
            if(($ExcludeList -ne $null) -and ($ExcludeList -ne "none"))
            {
                ...
                {
		            $currentVM = $AllVMs | where Name -Like $filtervm.Trim()  -ErrorAction SilentlyContinue
...
function PerformActionOnSequencedTaggedVMRGs($Sequences, [string]$Action, $TagName, [string[]]$VMRGList, $ExcludeList)
{
    ...
    foreach($seq in $Sequences)
    {
        ...
        if($WhatIf -eq $false)
        {
			...
			{
                $AzureVMList += $AllVMResources | where ResourceGroupName -eq $rg.Trim() | Select Name, ResourceGroupName
            }
...
function PerformActionOnSequencedTaggedVMList($Sequences, [string]$Action, $TagName, [string[]]$AzVMList)
{
    ...
    foreach($seq in $Sequences)
    {
        ...
        if($WhatIf -eq $false)
        {
			...
			{
                $AzureVMList += $AllVMResources | where Name -eq $vm.Trim() | Select Name, ResourceGroupName
            }
